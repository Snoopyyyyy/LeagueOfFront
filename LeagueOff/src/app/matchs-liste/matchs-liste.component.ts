...
export class MatchsListeComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.summonerService.getSummoner(name!).subscribe({
			next: (summoner) => {
				this.summoner = summoner;
				this.gameService.getHistory(summoner.puuid).subscribe({
					next: (games) => {
						this.games = games.map((game) => {
							game.currentPlayer = game.players.find(
								(ply) => ply.summonerName === this.name
							)!;
	...
	getKda(game: Game): number {
		let kda =
			(game.currentPlayer.kills! + game.currentPlayer.assists!) /
			game.currentPlayer.deaths!;
	...
	getPourCent(value: number): number {
		return (
			Math.round(((value * 100) / (this.showGames?.length ?? 0)) * 100) / 100
		);
	}
	...
	reloadStat() {
		...
		for (let champName in champs) {
			champs[champName].cs =
				Math.round((champs[champName].cs / champs[champName].nb_game) * 100) /
				100;
			champs[champName].kill =
				Math.round((champs[champName].kill / champs[champName].nb_game) * 100) /
				100;
			champs[champName].assists =
				Math.round(
					(champs[champName].assists / champs[champName].nb_game) * 100
				) / 100;
			champs[champName].deaths =
				Math.round(
					(champs[champName].deaths / champs[champName].nb_game) * 100
				) / 100;
			champs[champName].win =
				Math.round(
					(champs[champName].win / champs[champName].nb_game) * 10000
				) / 100;
			champs[champName].loose =
				Math.round(
					(champs[champName].loose / champs[champName].nb_game) * 10000
				) / 100;
			let kda =
				(champs[champName].kill + champs[champName].assists) /
				champs[champName].deaths;
