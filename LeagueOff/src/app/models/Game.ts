...
export class Game {
	...
	constructor(game: Game) {
		...
		this.buildings = cleanBuildings[game.gameMode].map((b: any) => {
		...
		this.buildings.push(
			...buildings
				.filter((b) => b.spawn !== 0)
				.map((b) => {
					let oldB = cleanBuildings[game.gameMode].find((bu: any) =>
						checkPosition(bu.position, b.position)
					);
					return oldB ? { ...b, position: oldB.position } : b;
				})
		);

		if (this.gameMode === "ARAM") this.buildings = this.buildings.map(b => {
			return {
				...b,
				position: {
					x: b.position.x * 1.21,
